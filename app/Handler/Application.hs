...
import           App.Component       (Component, FlexComp, getCookie,
                                      getSessionStore)
...
pager current total =
  ...
  where
    buildClass p =
      if p == current
        then Text.pack "current"
        else Text.pack "non-current"
    buildUri p = Text.pack ("/page/" ++ show p)
    pages = [0 .. (totalPage - 1)]
    totalPage = quot (total + Config.articlePerPage - 1) Config.articlePerPage

isUser :: Text -> FlexComp Bool
isUser email = do
...
github :: Component
github =
  [parse|svg { xmlns="http://www.w3.org/2000/svg", viewBox="0 0 48 48", class="github-icon" }
